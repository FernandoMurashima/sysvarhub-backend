...
class Command(BaseCommand):
    help = "Sincroniza vendedores locais do Sysvar Hub a partir da retaguarda."
    ...
    def handle(self, *args, **options):
        ...
        try:
            resposta = client.vendedores(token=hub.retaguarda_token)
            resultado = sincronizar_vendedores(hub, resposta)
        except (RetaguardaError, VendedoresValidationError) as exc:
            raise CommandError(str(exc)) from exc

        self.stdout.write(self.style.SUCCESS("Vendedores sincronizados com sucesso."))
        ...
        self.stdout.write(f"Vendedores recebidos: {resultado['vendedores_recebidos']}")
        self.stdout.write(f"Vendedores ausentes marcados: {resultado['vendedores_ausentes_marcados']}")
